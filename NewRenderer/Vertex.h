...
namespace std {
template<>
struct hash<Farlor::VertexPositionUVNormal> {
    size_t operator()(Farlor::VertexPositionUVNormal const &vertex) const
    {
        return ((hash<Farlor::Vector3>()(vertex.m_position)
                      ^ (hash<Farlor::Vector3>()(vertex.m_normal) << 1))
                     >> 1)
              ^ (hash<Farlor::Vector2>()(vertex.m_uv) << 1);
    }
};
}
