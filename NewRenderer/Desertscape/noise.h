...
static int Perm[512] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140,
    ...
    120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117, 35, 11, 32, 57, 177, 33, 88, 237, 149, 56,
    87, 174, 20, 125, 136, 171, 168, 68, 175, 74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231,
...
class PerlinNoise {
   public:
    ...
    static inline float GetValue(const Math::Vector2 &p) { return GetValue(p.ToVector3(0.0f)); }

    static inline float GetValue(const Math::Vector3 &p)
    {
        float x = p.x;
        float y = p.y;
        float z = p.z;
        ...
        // Interpolate results
        const float l1
              = Math::Lerp(Gradient(Perm[aa], x, y, z), Gradient(Perm[ba], x - 1, y, z), u);
        const float l2
              = Math::Lerp(Gradient(Perm[ab], x, y - 1, z), Gradient(Perm[bb], x - 1, y - 1, z), u);
        const float l3 = Math::Lerp(
              Gradient(Perm[aa + 1], x, y, z - 1), Gradient(Perm[ba + 1], x - 1, y, z - 1), u);
        const float l4 = Math::Lerp(Gradient(Perm[ab + 1], x, y - 1, z - 1),
              Gradient(Perm[bb + 1], x - 1, y - 1, z - 1), u);
    ...
    static inline float fBm(const Math::Vector3 &p, float a, float f, int o)
    {
        float ret = 0.0f;
        float freq = f;
        float amp = a;
