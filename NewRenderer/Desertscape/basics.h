...
class Random {
	...
	*/
	...
	\param b max
	*/
    static inline float Uniform(float a, float b) { return a + (b - a) * Uniform(); }
...
class Box2D {
   ...
   public:
    ...
    explicit Box2D(const Math::Vector2 &A, const Math::Vector2 &B);
    explicit Box2D(const Math::Vector2 &C, float R);

    float Width() const;
    Math::Vector2 Size() const;
    Math::Vector2 Vertex(int i) const;
    Math::Vector2 Center() const;
    Math::Vector2 BottomLeft() const;
    Math::Vector2 TopRight() const;
    Math::Vector2 &operator[](int i);
    Math::Vector2 operator[](int i) const;
};
...
{
    a = Math::Vector2::Min(b1.a, b2.a);
    b = Math::Vector2::Max(b1.b, b2.b);
}
...
*/
inline Box2D::Box2D(const Math::Vector2 &A, const Math::Vector2 &B)
    : a(A)
    , b(B)
{
}
...
\param R radius
*/
inline Box2D::Box2D(const Math::Vector2 &C, float R)
{
    Math::Vector2 RR = Math::Vector2(R);
    a = C - RR;
    b = C + RR;
}
...
*/
inline Math::Vector2 Box2D::Vertex(int i) const
{
    if (i == 0)
        return a;
    return b;
...
*/
inline Math::Vector2 Box2D::Center() const { return (a + b) / 2; }

/*
\brief Get bottom left vertex in world coordinates
*/
inline Math::Vector2 Box2D::BottomLeft() const { return a; }

/*
\brief Get top right vertex in world coordinates
*/
inline Math::Vector2 Box2D::TopRight() const { return b; }

/*!
\brief Compute the size of the box.
*/
inline Math::Vector2 Box2D::Size() const { return b - a; }

/*!
\brief Compute the width of a box.
\sa Box2D::Size()
*/
inline float Box2D::Width() const { return b[0] - a[0]; }

/*
\brief Access box vertex by reference
*/
inline Math::Vector2 &Box2D::operator[](int i)
{
    if (i == 0)
        return a;
    return b;
...
*/
inline Math::Vector2 Box2D::operator[](int i) const
{
    if (i == 0)
        return a;
    return b;
...
class ScalarField2D {
	...
	*/
    inline ScalarField2D(int nx, int ny, const Box2D &bbox)
        : box(bbox)
        , nx(nx)
        , ny(ny)
    {
	...
	*/
    inline ScalarField2D(int nx, int ny, const Box2D &bbox, float value)
        : box(bbox)
        , nx(nx)
        , ny(ny)
    {
	...
	*/
    inline ScalarField2D(const ScalarField2D &field)
        : ScalarField2D(field.nx, field.ny, field.box)
    {
	...
	*/
    inline Math::Vector2 Gradient(int i, int j) const
    {
        Math::Vector2 ret;
	...
	*/
    inline Math::Vector3 Vertex(const Math::Vector2 &v) const
    {
        return Math::Vector3(v.x, GetValueBilinear(v), v.y);
    }

    /*!
	\brief Check if a point lies inside the bounding box of the field.
	*/
    inline bool Inside(const Math::Vector2 &p) const
    {
        Math::Vector2 q = p - box.Vertex(0);
        Math::Vector2 d = box.Vertex(1) - box.Vertex(0);
	...
	*/
	...
	\brief Utility.
	*/
    inline int ToIndex1D(const Math::Vector2i &v) const { return v.x * nx + v.y; }
	...
	*/
	...
	\brief Todo
	*/
    inline void CellInteger(const Math::Vector2 &p, int &i, int &j) const
    {
        Math::Vector2 q = p - box.BottomLeft();
        Math::Vector2 d = box.Size();
	...
	*/
	...
	\brief Returns the value of the field at a given coordinate.
	*/
    inline float Get(int index) const { return values[index]; }

    /*!
	\brief Returns the value of the field at a given coordinate.
	*/
    inline float Get(const Math::Vector2i &v) const
    {
	...
	\brief Todo
	*/
    void Add(int i, int j, float v) { values[ToIndex1D(i, j)] += v; }

    /*!
	\brief Todo
	*/
    void Remove(int i, int j, float v) { values[ToIndex1D(i, j)] -= v; }
	...
	*/
	...
	\param p world point.
	*/
    inline float GetValueBilinear(const Math::Vector2 &p) const
    {
        Math::Vector2 q = p - box.Vertex(0);
        Math::Vector2 d = box.Vertex(1) - box.Vertex(0);
        ...
        return (1 - localU) * (1 - localV) * v1 + (1 - localU) * localV * v2
              + localU * (1 - localV) * v4 + localU * localV * v3;
    }

    /*!
	\brief Fill all the field with a given value.
	*/
    inline void Fill(float v) { std::fill(values.begin(), values.end(), v); }

    /*!
	\brief Return the data in the field.
	\param c Index.
	*/
    inline float &operator[](int c) { return values[c]; }

    /*!
	\brief Set a given value at a given coordinate.
	*/
    inline void Set(int row, int column, float v) { values[ToIndex1D(row, column)] = v; }

    /*!
	\brief Set a given value at a given coordinate.
	*/
    inline void Set(int index, float v) { values[index] = v; }
	...
	*/
    inline Math::Vector2 BottomLeft() const { return box.Vertex(0); }

    /*!
	\brief Returns the top right corner of the bounding box.
	*/
    inline Math::Vector2 TopRight() const { return box.Vertex(1); }
	...
	*/
    inline int Memory() const { return sizeof(ScalarField2D) + sizeof(float) * int(values.size()); }
};
