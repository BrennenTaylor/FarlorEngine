...
inline float QuinticSmooth(float t) { return pow(t, 3.0f) * (t * (t * 6.0f - 15.0f) + 10.0f); }
...
struct Vector2i {
   public:
    ...
    Vector2i operator-(const Vector2i &u) const { return Vector2i(x - u.x, y - u.y); }
    Vector2i operator+(const Vector2i &u) const { return Vector2i(x + u.x, y + u.y); }
};
...
struct Vector3 {
   public:
    ...
    inline float Max() const { return Math::Max(Math::Max(x, y), z); }
    inline float Min() const { return Math::Min(Math::Min(x, y), z); }
...
inline float Magnitude(const Vector3 &u) { return sqrt(u.x * u.x + u.y * u.y + u.z * u.z); }
inline float SquaredMagnitude(const Vector3 &u) { return u.x * u.x + u.y * u.y + u.z * u.z; }
...
struct Vector2 {
   public:
    ...
    inline Vector3 ToVector3(float z) const { return Vector3(x, z, y); }
    inline float Max() const { return Math::Max(x, y); }
    inline float Min() const { return Math::Min(x, y); }
...
inline float Dot(const Vector2 &u, const Vector2 &v) { return u.x * v.x + u.y * v.y; }
inline float Magnitude(const Vector2 &u) { return sqrt(u.x * u.x + u.y * u.y); }
inline float SquaredMagnitude(const Vector2 &u) { return u.x * u.x + u.y * u.y; }
...
inline bool operator>=(const Vector2 &u, const Vector2 &v) { return (u.x >= v.x) && (u.y >= v.y); }
inline bool operator<=(const Vector2 &u, const Vector2 &v) { return (u.x <= v.x) && (u.y <= v.y); }
inline Vector2 operator*(const Vector2 &u, float a) { return Vector2(u.x * a, u.y * a); }
inline Vector2 operator*(float a, const Vector2 &v) { return Vector2(v.x * a, v.y * a); }
